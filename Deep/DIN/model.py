...
class Model(object):
    ...
    def train(self,sess,uij,l):
        loss,_ = sess.run([self.loss,self.train_op],feed_dict={
            ...
            self.i : uij[1],
            self.y : uij[2],
            self.hist_i : uij[3], #hist_i 用0进行补齐
            self.sl : uij[4],
            self.lr : l
        })
...
def attention(queries,keys,keys_length):
    ...
    queries = tf.tile(queries,[1,tf.shape(keys)[1]]) # 数据平铺成 B, H*T
